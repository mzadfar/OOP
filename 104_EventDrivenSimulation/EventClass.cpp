...
int EventClass::getID() const { return customerID; }

int EventClass::getType() const { return eventType; }

int EventClass::getTime() const { return endTimeOfEvent; }

void EventClass::setTime(int time) { endTimeOfEvent = time; }
...
ostream &operator<<(ostream &os, const EventClass &iObj) {
  os << " Cusomer ID: " << iObj.getID() << " Happen Time: " << iObj.getTime()
     << " Event Type: ";
