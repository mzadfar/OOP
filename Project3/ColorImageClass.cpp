...
//Purpose: develop program for member functions of image class 
//to modifying pictures on a computer

//Implementing functions of ColorImageClass:
//A private function to get and validate row and column size from a file 
...
//A private function to check validation of input PPM file, and xtract
...
//A private fuction to write all pixels in the image into a file  
...
{
  rowSize = ROW_INITIAL_VAL;
  colSize = COL_INITIAL_VAL;
  arr = NULL;
}
/*
                                  ...
                                  RowColumnClass &inRowColLwrRight)
{
  ...
  int rectSpecMtd, ctrRowRec, ctrClmnRec, numRowRec, numClmnRec, hlfNumRowRec,
      hlfNumClmnRec, recOptnCnt = MENU_START_VAL;
  ...
  if (rectSpecMtd == UP_LFT_LOW_RGHT_CRNR)
  {
  ...
  else if (rectSpecMtd == UP_LFT_CRNR_ROW_COL_DIMEN) 
  {
    cout << "Enter upper left corner row and column: ";
    cin >> upLeftRecRow;
    cin >> upLeftRecClmn; 
  ...
  else if (rectSpecMtd == EXTENT_FRM_CNTR) 
  {
...
{
  int colorChoiceInt, colorOptnCnt = MENU_START_VAL;
  ...
  if (mainMenuChoic == RECT_COLOR)
    cout << "Enter int for rectangle color: ";
  if (mainMenuChoic == PATTRN_COLOR)
    cout << "Enter int for pattern color: ";
  if (mainMenuChoic == TRANSPAR_COLOR)
    cout << "Enter int for transparecy color: ";

  validIntInput(colorChoiceInt, colorOptnCnt);

  if (colorChoiceInt == SET_TO_RED)
  {
    outColor.setToRed();
  }
  if (colorChoiceInt == SET_TO_GREEN)
  {
    outColor.setToGreen();
  }
  if (colorChoiceInt == SET_TO_BLUE)
  {
    outColor.setToBlue();
  }
  if (colorChoiceInt == SET_TO_BLACK)
  {
    outColor.setToBlack();
  }
  if (colorChoiceInt == SET_TO_WHITE)
  {
    outColor.setToWhite();
  }
}
...
{
  int recFillOptn, recFillOptnCnt = MENU_START_VAL;
  int upLeftRecRow = inRowColUpLeft.getRow();
  int upLeftRecClmn = inRowColUpLeft.getCol();
  int lwrRightRecRow = inRowColLwrRight.getRow();
  int lwrRightRecClmn = inRowColLwrRight.getCol();
