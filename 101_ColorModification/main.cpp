...
Colors are typically described as a combination of some amount of red, green,
and blue (referred to as “RGB” values). With different amounts of red, green,
and blue, you can create any color. For example, the color bright red would be
represented as a full amount of red and no green and no blue, and a dimmer red
would be represented as less than a full amount of red with no green and no
blue.

Similarly, black is no red, no green, and no blue, while white is full red, full
green, and full blue. Other colors are obtained by mixing the component colors,
so yellow is obtained via full red, full green, and no blue, etc. The component
color values must be within a specified range, and the term “full red” (or green
or blue) means the maximum allowed color value, which for this project will be
1000, while the minimum allowed color value is 0.

An image is just a bunch of colors arranged in a rectangle. Because each
individual color element (called a pixel) is so small, your eyes and brain can
interpret the rectangle of tiny pixel colors as an image. Since an image is made
up some number of rows and columns, the pixel color values are often stored as a
twodimensional array (rows being the first dimension, and columns being the
second dimension).

Finally, we may want to refer to a specific pixel within the collection of
pixels that make up an image. We can do this with a row index and a column index
to uniquely identify a specific pixel via its location.

For this project, we will implement the beginnings of three classes to represent
colors, images, and locations within images.
 */
...
int main() {
  ...
  cout << "----------Test some basic RowColClass operations----------" << endl;
  ...
  cout << "----------Test some operations of ColorImageClass----------" << endl;
  testColor.setToRed();
  testImage.initializeTo(testColor);

  testRowCol.setRowCol(ROW_TEST_VAL, ((COL_TEST_VAL >> 3) + 1));
  cout << "Want color at [" << ROW_TEST_VAL << ", " << ((COL_TEST_VAL >> 3) + 1)
       << "]: Invalid Index!" << endl;
...
bool ColorClass::clipColorVal(int &inVal1, int &inVal2, int &inVal3) {
  bool retVal =
      ((inVal1 < MIN_ALLOWED_COLOR_VAL) || (inVal1 > MAX_ALLOWED_COLOR_VAL) ||
       (inVal2 < MIN_ALLOWED_COLOR_VAL) || (inVal2 > MAX_ALLOWED_COLOR_VAL) ||
       (inVal3 < MIN_ALLOWED_COLOR_VAL) || (inVal3 > MAX_ALLOWED_COLOR_VAL));
...
// to the input values
void RowColumnClass::setRow(int inRowNew) { inRow = inRowNew; }

// A public fuction to set the column value of the object
// to the input values
void RowColumnClass::setCol(int inColNew) { inCol = inColNew; }

// A public fuction to return the row value of the object
// to the caller
int RowColumnClass::getRow() { return inRow; }

// A public fuction to return the column value of the object
// to the caller
int RowColumnClass::getCol() { return inCol; }
